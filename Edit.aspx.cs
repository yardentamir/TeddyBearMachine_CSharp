...
public partial class Edit : System.Web.UI.Page
{
    ...
    protected void UpdateBtn_Click(object sender, EventArgs e)
    {
        ...
        XmlNodeList mynodes = myDoc.SelectNodes("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']//Ans");
        foreach (XmlNode mynode in mynodes)
        {
        ...
        if (fileInput.HasFile)
        {
            ...
            if (fileType.Contains("image")) //בדיקה האם הקובץ שהוכנס הוא תמונה
            {
                ...
                XmlNode myQelemnt = myDoc.SelectSingleNode("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']");
                myQelemnt.Attributes["text"].Value = Server.UrlEncode(((TextBox)FindControl("Question")).Text);
                myQelemnt.Attributes["pic"].Value = imageNewName;
        ...
        {
            if (((ImageButton)FindControl("ShowPic0")).ImageUrl.Length > 1)
            { // אם אין תוכן בהעלאת תמונה אבל יש תמונה בתצוגה
                XmlNode myQelemnt = myDoc.SelectSingleNode("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']");
                myQelemnt.Attributes["text"].Value = Server.UrlEncode(((TextBox)FindControl("Question")).Text);
                myQelemnt.Attributes["pic"].Value = ((ImageButton)FindControl("ShowPic0")).ImageUrl;
            }
            else
            {
                XmlNode myQelemnt = myDoc.SelectSingleNode("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']");
                myQelemnt.Attributes["text"].Value = Server.UrlEncode(((TextBox)FindControl("Question")).Text);
                myQelemnt.Attributes["pic"].Value = "False";
            }
        }
        myDoc.Save(Server.MapPath("trees/XMLFile1.xml"));
        ...
        for (int i = 2; i < 6; i++)
        {
            ...
            { // יש טקסט במסיחים
                ...
                if (((TextBox)FindControl("Ans" + q)).Text != "")
                {
                    ...
                    if (q == 0) Answer.SetAttribute("cor", "true");
                    else Answer.SetAttribute("cor", "false");
                    XmlNode myQelemnt = myDoc.SelectSingleNode("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']");
                ...
                string me = ((ImageButton)FindControl("ShowPic" + (i - 1))).ImageUrl;
                if (me.Length > 1 && i != isFileInputAndPicture)
                {
                    XmlElement Answer = myDoc.CreateElement("Ans");
                    XmlText Anst2;
                    if (i == 2) Answer.SetAttribute("cor", "true");
                    else Answer.SetAttribute("cor", "false");
                    Anst2 = myDoc.CreateTextNode(((ImageButton)FindControl("ShowPic" + (i - 1))).ImageUrl);
                    XmlNode myQelemnt = myDoc.SelectSingleNode("/games/game[@id='" + theItemId + "']/question[@id='" + myID + "']");
                    Answer.SetAttribute("anstype", "pic");
                    Answer.AppendChild(Anst2);
                    myQelemnt.AppendChild(Answer);
        ...
        SaveQ.Style.Add("visibility", "true");
        UpdateBtn.Style.Add("visibility", "hidden");
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            ...
            case "editRow":
                ...
                showQ(theId);
                SaveQ.Style.Add("visibility", "hidden");
                UpdateBtn.Style.Add("visibility", "visible");
    ...
    protected void showQ(string myId)
    {
        ...
        {
            ((FileUpload)FindControl("fileInput" + i)).Dispose();
            //((FileUpload)FindControl("fileInput" + i)).Attributes.Clear();
        }
        for (int i = 0; i < 4; i++)
        {
            ((System.Web.UI.WebControls.Image)FindControl("pic" + i)).Style.Add("visibility", "visible");
        ...
        string theItemId = Session["theItemIdSession"].ToString();
        XmlNodeList myNodes0 = myDoc.SelectNodes("//games/game[@id='" + theItemId + "']/question[@id='" + myId + "']");
        foreach (XmlNode myNode in myNodes0)
        {
            ...
            if (myNode.Attributes["pic"].Value != "False")
            {
                ...
                //((FileUpload)FindControl("fileInput")).PostedFile.FileName(myNode.Attributes["pic"].Value); 
        ...
        foreach (XmlNode myNode6 in myNodes6)
        {
            ...
            {
                string ms = myNode6.InnerXml;
                ((ImageButton)FindControl("ShowPic" + counterp2)).ImageUrl = myNode6.InnerXml;
                //((FileUpload)FindControl("fileInput" + forfilesCounter)).PostedFile.SaveAs(Server.MapPath(myNode6.InnerText));
            ...
            else  /// כלומר טקסט
            {
                ...
                ((ImageButton)FindControl("Deletebtn" + counterp2)).Style.Add("visibility", "hidden");
                ((System.Web.UI.WebControls.Image)FindControl("pic" + uploadIconCounter)).Style.Add("visibility", "hidden");
            }
            counterp2++;
            forfilesCounter++;
    ...
    void deleteRow(string theItemId, string theId)
    {
        ...
        SaveQ.Style.Add("visibility", "visible");
        UpdateBtn.Style.Add("visibility", "hidden");
        int idcounter = -1;
        XmlNodeList myNodes;
        myNodes = myDoc.SelectNodes("//games/game[@id='" + theItemId + "']//question");
    ...
    protected void ClickPach(string pach, string showPic, string fileInput, string pic, string answer)
    {
        ((ImageButton)FindControl(showPic)).ImageUrl = "";
        ((ImageButton)FindControl(showPic)).Style.Add("visibility", "hidden");
        ((ImageButton)FindControl(pach)).Style.Add("visibility", "hidden");
        ((FileUpload)FindControl(fileInput)).Dispose();
        ((System.Web.UI.WebControls.Image)FindControl(pic)).Style.Add("visibility", "visible");
        ((TextBox)FindControl(answer)).Enabled = true;
    }
